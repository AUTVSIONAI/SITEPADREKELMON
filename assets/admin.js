...
const loadEvents=async()=>{try{const api=localStorage.getItem('API_BASE')||API_BASE;const r=await fetch(`${api}/api/admin/events`,{headers:h()});if(r.status===401){eventsList.innerHTML='<div class="form-status">Token de Admin inválido ou ausente. Abra Conexão, salve o token e recarregue.</div>';return}const list=r.ok?await r.json():[];eventsList.innerHTML=list.map(e=>{const valDate=(()=>{try{const d=new Date(e.date);return new Date(d.getTime()-d.getTimezoneOffset()*60000).toISOString().slice(0,16)}catch{return ''}})();return `<div class=\"agenda-item\"><div class=\"form-group\"><label>Título</label><input type=\"text\" value=\"${e.title||''}\" data-e-title=\"${e.id}\"></div><div class=\"form-group\"><label>Data</label><input type=\"datetime-local\" value=\"${valDate}\" data-e-date=\"${e.id}\"></div><div class=\"form-group\"><label>Local</label><input type=\"text\" value=\"${e.place||''}\" data-e-place=\"${e.id}\"></div><div class=\"form-group\"><label>Tipo</label><select data-e-type=\"${e.id}\"><option ${e.type==='missa'?'selected':''} value=\"missa\">Missa</option><option ${e.type==='evento'?'selected':''} value=\"evento\">Evento</option><option ${e.type==='reuniao'?'selected':''} value=\"reuniao\">Reunião</option><option ${e.type==='entrevista'?'selected':''} value=\"entrevista\">Entrevista</option></select></div><div class=\"form-group\"><label>Descrição</label><textarea rows=\"3\" data-e-desc=\"${e.id}\">${e.description||''}</textarea></div><div class=\"form-group inline\"><input type=\"checkbox\" ${e.published?'checked':''} data-e-pub=\"${e.id}\"><label>Publicado</label></div><div class=\"filters\"><button class=\"btn btn-outline\" data-e-save=\"${e.id}\">Salvar</button><button class=\"btn btn-outline\" data-act=\"delete\" data-id=\"${e.id}\">Excluir</button></div></div>`}).join('');eventsList.querySelectorAll('[data-e-save]').forEach(b=>b.addEventListener('click',async()=>{const id=b.getAttribute('data-e-save');const api2=localStorage.getItem('API_BASE')||API_BASE;const body={title:eventsList.querySelector(`[data-e-title=\"${id}\"]`).value,date:new Date(eventsList.querySelector(`[data-e-date=\"${id}\"]`).value).toISOString(),place:eventsList.querySelector(`[data-e-place=\"${id}\"]`).value,type:eventsList.querySelector(`[data-e-type=\"${id}\"]`).value,description:eventsList.querySelector(`[data-e-desc=\"${id}\"]`).value,published:eventsList.querySelector(`[data-e-pub=\"${id}\"]`).checked};const r2=await fetch(`${api2}/api/admin/events/${id}`,{method:'PATCH',headers:h(),body:JSON.stringify(body)});if(r2.ok)loadEvents()}));eventsList.querySelectorAll('[data-act]').forEach(b=>b.addEventListener('click',async()=>{const id=b.getAttribute('data-id');const act=b.getAttribute('data-act');const api3=localStorage.getItem('API_BASE')||API_BASE;if(act==='delete'){const rd=await fetch(`${api3}/api/admin/events/${id}`,{method:'DELETE',headers:h()});if(rd.ok)loadEvents()}}))}catch{eventsList.innerHTML=''} }
